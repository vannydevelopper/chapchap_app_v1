...
export default function AddCart({ combinaisons, onPress, variantes, colors, SIZES, product, loadingForm, onClose }) {
                ...
                <>
                  ...
                  <View style={[styles.sizes]}>
                    {variante.values.map((value, index) => {
                      const isSelected = combinaisons.find(i => i.ID_VALUE == value.ID_VALUE)?true:false

                      return (
                        <TouchableOpacity style={[styles.size, index == 0 && { marginLeft: 0 }, isSelected && { backgroundColor: COLORS.ecommercePrimaryColor }]} key={index} onPress={() => {
                          // onChangeText(0)
                          onPress(value,variante.values.length)
                        }
                        }>
                          <Text style={[styles.sizeText, isSelected && { color: '#FFF' }]}>{value.VALUES_NAME}</Text>
                        </TouchableOpacity>
                      )
                    }
